import { getContextIdentifiers, getMetaSchemaNode, getMetaSchemaNodeProperty, getRuleInfo, hasUnresolvedObjectSpread, insertProperty, isUndefinedIdentifier } from "../utils.mjs";
...
const rule = {
	...
	create(context) {
		...
		return {
			...
			"Program:exit"() {
				if (!schemaNode && !metaMayHaveSchema && requireSchemaPropertyWhenOptionless) context.report({
			...
			MemberExpression(node) {
				if ((hasEmptySchema || !schemaNode && !metaMayHaveSchema) && node.object.type === "Identifier" && contextIdentifiers.has(node.object) && node.property.type === "Identifier" && node.property.name === "options") {
					isUsingOptions = true;
					context.report({

import { evaluateObjectProperties, getKeyName, getRuleInfo, hasUnresolvedObjectSpread } from "../utils.mjs";
import { getStaticValue } from "@eslint-community/eslint-utils";
//#region lib/rules/require-meta-type.ts
/**
...
const rule = {
	...
	create(context) {
		...
		return { Program(ast) {
			...
			if (!typeNode) {
				if (hasUnresolvedObjectSpread(metaNode, scopeManager)) return;
				context.report({
			...
			if (typeof staticValue.value !== "string" || !VALID_TYPES.has(staticValue.value)) context.report({
				node: typeNode.value,

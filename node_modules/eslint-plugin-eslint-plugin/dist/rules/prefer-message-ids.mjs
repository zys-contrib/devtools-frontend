import { collectReportViolationAndSuggestionData, evaluateObjectProperties, getContextIdentifiers, getKeyName, getReportInfo, getRuleInfo, hasUnresolvedObjectSpread } from "../utils.mjs";
	...
	create(context) {
		...
		return {
			Program(ast) {
				...
				const metaProperties = evaluateObjectProperties(metaNode, sourceCode.scopeManager);
				const messagesNode = metaNode && metaNode.type === "ObjectExpression" && metaProperties.filter((p) => p.type === "Property").find((p) => getKeyName(p) === "messages");
				if (!messagesNode) {
					if (hasUnresolvedObjectSpread(metaNode, sourceCode.scopeManager)) return;
					context.report({

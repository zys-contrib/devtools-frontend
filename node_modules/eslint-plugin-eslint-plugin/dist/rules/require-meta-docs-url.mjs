import { getMetaDocsProperty, getRuleInfo, insertProperty, isUndefinedIdentifier } from "../utils.mjs";
	...
	create(context) {
		...
		*/
		...
		return { Program(ast) {
			...
			const { docsMayHaveUnknownProperties, docsNode, metaNode, metaPropertyNode: urlPropNode } = getMetaDocsProperty("url", ruleInfo, scopeManager);
			const staticValue = urlPropNode ? getStaticValue(urlPropNode.value, scope) : void 0;
			if (urlPropNode && !staticValue) return;
			if (!urlPropNode && docsMayHaveUnknownProperties) return;

import { evaluateObjectProperties, getContextIdentifiers, getKeyName, getRuleInfo, hasUnresolvedObjectSpread } from "../utils.mjs";
	...
	create(context) {
		...
		return {
			...
			"Program:exit"(ast) {
				...
				const metaProperties = evaluateObjectProperties(ruleInfo.meta, scopeManager);
				const metaFixableProp = ruleInfo && metaProperties.filter((prop) => prop.type === "Property").find((prop) => getKeyName(prop) === "fixable");
				if (metaFixableProp) {
				...
				} else if (!metaFixableProp && usesFixFunctions && !hasUnresolvedObjectSpread(ruleInfo.meta, scopeManager)) context.report({

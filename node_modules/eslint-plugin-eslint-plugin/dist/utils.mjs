...
* @param scopeManager
*/
function resolveSpreadObject(argument, scopeManager) {
	let node = argument;
	if (node.type === "Identifier" && isUndefinedIdentifier(node)) return { kind: "empty" };
	...
	if (SPREAD_TYPES_WITHOUT_NAMED_PROPERTIES.has(node.type)) return { kind: "empty" };
	return { kind: "unknown" };
}
/**
...
function evaluateObjectProperties(objectNode, scopeManager) {
	return deduplicatePropertiesKeepingLast(evaluateObjectPropertiesWithVisited(objectNode, scopeManager, /* @__PURE__ */ new Set()));
}
function evaluateObjectPropertiesWithVisited(objectNode, scopeManager, visited) {
	if (!objectNode || objectNode.type !== "ObjectExpression" || visited.has(objectNode)) return [];
	...
	const properties = objectNode.properties.flatMap((property) => {
		if (property.type === "SpreadElement") {
			...
			return resolution.kind === "object" ? evaluateObjectPropertiesWithVisited(resolution.node, scopeManager, visited) : [];
		}
		return [property];
	});
...
* @param properties
*/
function deduplicatePropertiesKeepingLast(properties) {
	const lastIndexByKey = /* @__PURE__ */ new Map();
	properties.forEach((property, index) => {
		if (property.type === "Property") {
			const keyName = getKeyName(property);
			if (keyName !== null) lastIndexByKey.set(keyName, index);
		}
	});
...
*/
function hasUnresolvedObjectSpread(objectNode, scopeManager) {
	return hasUnresolvedObjectSpreadWithVisited(objectNode, scopeManager, /* @__PURE__ */ new Set());
}
function hasUnresolvedObjectSpreadWithVisited(objectNode, scopeManager, visited) {
	...
	const hasUnresolvedSpread = objectNode.properties.some((property) => {
		...
		const resolution = resolveSpreadObject(property.argument, scopeManager);
		if (resolution.kind === "unknown") return true;
		if (resolution.kind === "empty") return false;
		return hasUnresolvedObjectSpreadWithVisited(resolution.node, scopeManager, visited);
	});
	visited.delete(objectNode);
...
function getMetaDocsProperty(propertyName, ruleInfo, scopeManager) {
	...
	const docsResolution = docsNode ? resolveSpreadObject(docsNode.value, scopeManager) : void 0;
	const docsObjectNode = docsResolution?.kind === "object" ? docsResolution.node : void 0;
	const metaPropertyNode = evaluateObjectProperties(docsObjectNode, scopeManager).filter((node) => node.type === "Property").find((p) => getKeyName(p) === propertyName);
	return {
		docsNode,
		docsMayHaveUnknownProperties: Boolean(docsNode && (docsResolution?.kind === "unknown" || hasUnresolvedObjectSpread(docsObjectNode, scopeManager))),
		metaNode,
		metaPropertyNode
	};
}
...
*/
function getTestInfoProperty(test, key) {
	if (test.type === "ObjectExpression") return test.properties.filter((item) => item.type === "Property").find((item) => item.key.type === "Identifier" && item.key.name === key || item.key.type === "Literal" && item.key.value === key) ?? null;
	return null;
}
/**
